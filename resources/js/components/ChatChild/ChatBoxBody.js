...
import { fetchMessages, loadMoreMessages } from "../Redux/Ducks/messagesDuck";
...
export default function ChatBoxBody({listenerState,listenerDispatch}) {
  ...
  const {messages,count,next,loading,fetching} = useSelector(state => state.messages)
  ...
  const loadMore = () => {
    dispatch(loadMoreMessages(next))
  };
  ...
  useEffect(() => {
    ...
    if(Object.keys(recipient).length != 0){
      dispatch(fetchMessages(recipient.id))
      .catch((e) => {
        console.log({e})
        if (e.errorCode == 404) {
            // setRuser(false);
                      ...
                      index == messages.length - 6 ? 
                        <VisibilitySensor onChange={onChange} offset={{ top: -400,}}>
                          <SingleMsg
                            ...
                            sender={msg.msg_from == user.id ? true : false}
...
function SingleMsg({ index, msg, sender,selectedUser }) {
